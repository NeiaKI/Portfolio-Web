          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              <div className="rounded-lg border border-border bg-background p-2.5 sm:p-3 flex flex-col gap-1 shadow-sm">
                ...
                <p className="text-xl sm:text-2xl font-bold text-foreground">{data.streak.toLocaleString()}</p>
                <p className="text-[11px] text-muted-foreground">Day Streak</p>
              </div>
              <div className="rounded-lg border border-border bg-background p-2.5 sm:p-3 flex flex-col gap-1 shadow-sm">
                <Trophy className="h-5 w-5 text-primary" />
                <p className="text-xl sm:text-2xl font-bold text-primary">{data.totalXp.toLocaleString()}</p>
                <p className="text-[11px] text-muted-foreground">Total XP</p>
              </div>
              <div className="rounded-lg border border-border bg-background p-2.5 sm:p-3 flex flex-col gap-1 shadow-sm">
                <BookOpen className="h-5 w-5 text-muted-foreground" />
                <p className="text-xl sm:text-2xl font-bold text-foreground">{data.courses.length}</p>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3">
              {data.courses.map((course) => (
                <div
                  key={course.lang}
                  className="rounded-lg border border-border bg-background px-3 py-2.5 flex items-center justify-between gap-2 shadow-sm"
                >
